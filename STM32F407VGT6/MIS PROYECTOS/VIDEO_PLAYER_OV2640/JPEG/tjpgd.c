...
#if (JD_FAST_STM32 == 1)
/*
 * Color conversion from YCbCr to R5G6B5
...
/*
* Color conversion from Y (brightness) to monochrome (grayscale) 16 bit
 * Copyright (C) 2023, VadRov, all right reserved.
* on simd for stm32
 */
...
/*
 * Convert color from YCbCr в R8G8B8
 * Copyright (C) 2023, VadRov, all right reserved.
 * on simd for stm32
 */
...
/*
 * Filling a memory area with a given word-wide value (32 bits)
 * Copyright (C) 2023, VadRov, all right reserved.
 */
...
/*
 * Filling a memory area with a given value half-word wide (16 bits)
 * Copyright (C) 2023, VadRov, all right reserved.
 */
...
/*
 * Filling a memory area with a given byte-wide (8-bit) value
 * Copyright (C) 2023, VadRov, all right reserved.
 */
...
/*
 * Copying one memory area to another (byte by byte)
 * Copyright (C) 2023, VadRov, all right reserved.
 */
...
/*
 * Copying one memory area to another (word by word)
 * Copyright (C) 2023, VadRov, all right reserved.
 */
...
/*
 * Inverse discrete cosine transform algorithm Arai (Arai Algorithm)
...
static inline void block_idct (int32_t* src, jd_yuv_t* dst)
{
	asm volatile(	"mov	r12, #8					\n\t"	//r12 = 8
					...
					"str	r2, [%[src], #32*4-4]	\n\t"
					"subs	r12, #1					\n\t"	//r12--
					"bne	1b						\n\t"	//r12 != 0 ? jump to label 1: exit the loop
					"mov	r12, #8					\n\t"	//r12 = 8
					"subs	%[src], #32				\n\t"	//src -= 32
					"2:								\n\t"	//метка 2
					"ldr	r2, [%[src]], #32		\n\t"	//v0, src += 4
					...
					"strh	r2, [%[dst], #4*2-16]	\n\t"
					"subs	r12, #1					\n\t"	//r12--
					"bne	2b						\n\t"	//r12 != 0 ? go to label 2: exit the loop
					:[src]"+r"(src), [dst]"+r"(dst)
					:
		...
		if (cmp && jd->ncomp != 3) {		/* Clear C blocks if not exist (monochrome image) */
#if (JD_FAST_STM32 == 0)
			for (i = 0; i < 64; bp[i++] = 128) ; //original
#else
			memset_32(bp, 0x00800080, 64/2); //optimization option
#endif
...
#else
			memset_32(&tmp[1], 0, 63); //optimization option
#endif
			...
			if ((jd->color_format != 2 && jd->color_format != 3) || !cmp) {	/* C components may not be processed if in grayscale output */
				if (z == 1 || (JD_USE_SCALE && jd->scale == 3)) {	/* If no AC element or scale ratio is 1/8, IDCT can be ommited and the block is filled with DC value */
					...
					if (JD_FASTDECODE >= 1) {
#if (JD_FAST_STM32 == 0)
						for (i = 0; i < 64; bp[i++] = d) ; //original
#else
						memset_32(bp, (d << 16) | d, 64/2); //optimization option
#endif
					} else {
#if (JD_FAST_STM32 == 0)
						memset(bp, d, 64); //original
#else
						//optimization
						d &= 0xff;
...
static inline JRESULT mcu_output (
	JDEC* jd,								//Pointer to a decoder object
	int (*outfunc)(JDEC*, void*, JRECT*),	//Pointer to the function of outputting a data block with an image to the display
	unsigned int x,							//Coordinates for outputting a data block with an image on the display
	unsigned int y
)
{
	//++++++++++++ workbuf for double buffer ++++++++++++
	...
	mx = jd->msx * 8; my = jd->msy * 8;					//Width and height of the image block, pixels
	rx = (x + mx <= jd->width) ? mx : jd->width - x;	//Width and height of the image block for output to the frame buffer (display)
	ry = (y + my <= jd->height) ? my : jd->height - y;	//The block may need to be "cropped" if it extends beyond the boundaries of the imag
	if (JD_USE_SCALE) {									//If scaling is enabled:
		rx >>= jd->scale; ry >>= jd->scale;				//We scale the block sizes for output to the frame buffer (on the display)
		if (!rx || !ry) return JDR_OK;					//If the width or height of the block to be output to the frame buffer (display) is 0, exit.
			x >>= jd->scale; y >>= jd->scale;			//Scaling the block coordinates.

	}
	rect.left = x; rect.right = x + rx - 1;				//Form the parameters (coordinates of the upper left and lower right corners)
	rect.top = y; rect.bottom = y + ry - 1;				//rectangular area with the image in the frame buffer (on the display)

	if (!JD_USE_SCALE || jd->scale != 3) {	//Not for scaling 1/8
		if (jd->color_format == 0 || jd->color_format == 1) {
			for (iy = 0; iy < my; iy++) {
				pc = py = jd->mcubuf;
				if (my == 16) {	//Double height block?
					pc += 64 * 4 + (iy >> 1) * 8;
					if (iy >= 8) py += 64;
				}
				else {	//Single height block
					pc += mx * 8 + iy * 8;
				}
				py += iy * 8;
				for (ix = 0; ix < mx; ix++) {
					cb = pc[0] - 128; 	//Get components Cb and Cr and remove offset */
					cr = pc[64] - 128;
					if (mx == 16) {					//Block with double width?
						if (ix == 8) py += 64 - 8;	/* Jump to next block if double block heigt */
						pc += ix & 1;				//Increment the color index for each odd pixel
					}
					else {						//Single width block
						pc++;					//Incrementing the color index
					}
					yy = *py++;			//We get the intensity component Y
			...
			asm volatile (	"movs 	r3, #0						\n\t" // initializing the counter in r3(iy) = 0
							"1:									\n\t" // bullet 1
							"add	r2, %[mcubuf], r3, lsl #3+1	\n\t" // initialization address in r2(py) = mcubuf + r3(iy)*8*2
																  	  // (index offset is 8, but in bytes it is 16, since the array is half words)
							"cmp	%[my], #16					\n\t" // comparison my c 16
							"bne	2f							\n\t" // if my != 16, then go to label 2
							"cmp	r3, #8						\n\t" // comparison r3(iy) with 8
							"it 	cs							\n\t" // it block for unsigned greater than or equal to condition (r3(iy) >= 8)
							"addcs	r2, #64*2					\n\t" // if greater than or equal to, then increase the address in r2(py) by the given offset
																  	  // (index offset 64, but in bytes it is 128, since the array is half words)
							"2:									\n\t" // bullet 2
							"movs	r1, #0						\n\t" // initializing the counter in r1(ix) = 0
							"3:									\n\t" // bullet 3
							"cmp	%[mx], #16					\n\t" // comparison mx c 16
							"bne	4f							\n\t" // if mx != 16, then go to label 4
							"cmp	r1, #8						\n\t" // comparison r1(ix) c 8
							"it 	eq							\n\t" // it-block for equal condition (r1(ix) == 8)
							"addeq	r2, #56*2					\n\t" // if equal, then increase the address in r2(py) by the given offset
																  	  // (index offset 56, but in bytes it is 112, since the array is half words)
							"4:									\n\t" // bullet 4
							"ldrsh	r0, [r2], #1*2				\n\t" // we read the intensity value into r0 as a signed half-word,
																  	  // at address in r2 (py), with post-increment (after reading) r2 by 2 (r0 = *py++)
							"cmp %[col_fmt], #2					\n\t"
							"bne 41f							\n\t"
																	  //Grayscale 8-bit, grayscale format 8-bit
							"usat 	r0, #8, r0					\n\t" // unsigned saturation (limitation) of the value in r0 to 8 bits r0 = usat(8, r0)
																  	  // (after executing the instruction in r0 the value is from 0 to 255)
							"strb	r0, [%[pix]], #1			\n\t" // write the value from r0 as a byte into memory at the address in pix,
																  	  // with post-increment (after writing) the address in pix by 1 (*pix++ = r0)
							"b		42f							\n\t"
							"41:								\n\t" //Grayscale 16-bit, format grayscale 16 bit
							"usat 	r0, #5, r0, asr #3			\n\t" // unsigned saturation (limitation) of the value in r0 to 5 digits with preliminary
																  	  // arithmetic shift to the right by 3 bits r0 = usat(5, r0 >> 3)
																  	  // (after executing the instruction in r0 the value is from 0 to 31)
							"orr	r0, r0, r0, lsl #6			\n\t" // Logical OR operation of a value in r0 with a value in r0 with a preliminary
																  	  // logical shift 6 bits to the left r0 = r0 | (r0 << 6)
							"orr 	r0, r0, r0, lsl #11			\n\t" // Logical OR operation of a value in r0 with a value in r0 with a preliminary
				  	  	  	  	  	  	  	  	  	  	  	  	  	  // logical shift 11 bits to the left r0 = r0 | (r0 << 11)
							"strh	r0, [%[pix]], #1*2			\n\t" // we write the value from r0, as a half-word, into memory at the address in pix,
																  	  // with post-increment (after writing) the address in pix by 2 (*pix++ = r0)
							"42:								\n\t"
							"adds	r1, #1						\n\t" //counter increment r1(ix)
							"cmp	r1, %[mx]					\n\t" // comparison r1 c mx
							"bcc	3b							\n\t" // if r1 < mx, then go to label 3
							"adds	r3, #1						\n\t" // counter increment r3(iy)
							"cmp	r3, %[my]					\n\t" // comparison r3 with my
							"bcc	1b							\n\t" // if r3 < my, then go to label 1
							:[pix]"+r"(pix)
		...
		if (JD_USE_SCALE && jd->scale) {
			...
			//Calculate the average values of the color components for each square corresponding to the pixel
			s = jd->scale * 2;	//Number of values to calculate the average
			w = 1 << jd->scale;	//Square size
			a = mx - w; //Number of bytes that must be skipped to move to the next line
			int k_byte = 1;
			...
			for (iy = 0; iy < my; iy += w) {
				for (ix = 0; ix < mx; ix += w) {
					...
					r = g = b = 0;
					for (y = 0; y < w; y++) {	//Accumulation of values of color components in a square  تراكم قيم مكونات اللون في المربع
						for (x = 0; x < w; x++) {
							if (jd->color_format == 0) { //RGB888 (24-bit/pix)
								r += *pix++;	//Accumulation R
								g += *pix++;	//Accumulation G
								b += *pix++;	//Accumulation B
							}
							else if (jd->color_format == 1) { //RGB565 (16-bit/pix)
								r += ((*(uint16_t*)pix) >> 11) & 0x1f;		//Accumulation R
								g += ((*(uint16_t*)pix) >> 6) & 0x1f;		//Accumulation G
								b += (*(uint16_t*)pix) & 0x1f;//Accumulation B
								pix += 2;
							}
							else if (jd->color_format == 2) { //Grayscale (8-bit/pix)
								r += *pix++;			//Accumulation Y (Grayscale 8-bit) */
							}
							else { //Grayscale_16 (16-bit/pix)
								r += (*((uint16_t*)pix)) & 0x1f;	//Accumulation Y (Grayscale 16-bit) */
								pix += 2;
							}
						}
						pix += a; //Move to next line
					}
					//Writing averaged values of color components to an array
					if (jd->color_format == 0) { //RGB888 (24-bit/pix)
						*op++ = (uint8_t)(r >> s);	//Accumulation R
						*op++ = (uint8_t)(g >> s);	//Accumulation G
						*op++ = (uint8_t)(b >> s);	//Accumulation B
					}
					else if (jd->color_format == 1) { //RGB565 (16-bit/pix)
						*((uint16_t*)op) = ((r >> s) << 11) |	//Accumulation R
										   ((g >> s) << 6)  |	//Accumulation G
										   (b >> s);		  	//Accumulation B
						op += 2;
					}
					else if (jd->color_format == 2) { //Grayscale (8-bit/pix)
						*op++ = (uint8_t)(r >> s);	/* average intensity Y (grayscale) */
					}
					else { //Grayscale_16 (16-bit/pix)
						r >>= s; //average intensity Y (grayscale)
						*((uint16_t*)op) = (r << 11) | (r << 6) | r; //the values of the components R, G, B are the same
						op += 2;
					}
				}
			}
	...
	else {	//For scaling factor only 1/8
		pc = jd->mcubuf + mx * my;
		cb = pc[0] - 128;	//Getting component values Cb/Cr
		cr = pc[64] - 128;
		for (iy = 0; iy < my; iy += 8) {
			py = jd->mcubuf;
			if (iy == 8) py += 64 * 2;
			for (ix = 0; ix < mx; ix += 8) {
				yy = *py;	//Getting the value of the Y component (intensity)
				py += 64;
				if (jd->color_format == 0) { //RGB888 (24-bit/pix)
					Convert_YCbCr_to_R8G8B8(yy, cb, cr, &r1, &g1, &b1); //Conversion YCbCr в R8G8B8
					*pix++ = r1;
					*pix++ = g1;
					*pix++ = b1;
				}
				else if (jd->color_format == 1) { //R5G6B5 (16-bit/pix)
					*((uint16_t*)pix) = Convert_YCbCr_to_R5G6B5(yy, cb, cr); //Conversion YCbCr в R5G6B5
					pix += 2;
				}
				else if (jd->color_format == 2) { //Grayscale (8-bit/pix)
					*pix++ = yy;
					...
					*((uint16_t*)pix) = Convert_Y_to_Grayscale16(yy); //Conversion Y в R5G6B5 (32 оттенка серого)
					pix += 2;
				}
			}
		}
	}
	//Trim the block if required.
	//Cut off the width of pixels that do not fall into the output area,
	//rebuilding the array with color information.
	mx >>= jd->scale;
	if (rx < mx) {	// if rx < mx, then rearrange the pixels to fit the output area
		uint8_t *s, *d;
		s = d = (uint8_t*)workbuf;
		if (jd->color_format == 1 || jd->color_format == 3) {	//R5G6B5 (16-bit/pix), Grayscale_16 (16-bit/pix)
			...
			mx *= 2;
			for (y = 1; y < ry; y++) { //Move pixels in the array for a new output area, starting from line 2
				memcpy_16(d += rx, s += mx, rx1);
			}
		}
		else { //R8G8B8, Crayscale 8-bit
			if (jd->color_format == 0) { //R8G8B8 - 3 bytes per pixel
				rx *= 3;
				mx *= 3;
			}
			for (y = 1; y < ry; y++) { //Move pixels in the array for a new output area, starting from line 2
