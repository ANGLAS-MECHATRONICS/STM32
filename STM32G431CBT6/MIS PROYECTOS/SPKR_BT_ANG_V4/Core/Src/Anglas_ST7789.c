...
void ST7789_Init(void)
{
	...
	//HAL_Delay(50);//DESCOMENTAR SI NO INICIA LA PANTALLA
...
void Measure_FPS_FillScreenColor(void){
    ...
    ST7789_Print(10, 30, BLACK, WHITE, Font_16x26, msg);  // (x, y, color_texto, color_fondo, fuente, texto)
    snprintf(msg, sizeof(msg), "FPS: %.2f", fps);
    ST7789_Print(10, 60, BLACK, WHITE, Font_16x26, msg);
...
void Measure_FPS_ImageBMP(void){
    ...
    char msg[64];
    t_start = HAL_GetTick();  // Tiempo inicial en milisegundos
    displayImage_fast("25.bmp");
    //displayImage_fast2("21.bmp");
    t_end = HAL_GetTick();    // Tiempo final
    uint32_t elapsed = t_end - t_start;
    float fps = 1000.0f / elapsed;
    // Formatear mensaje
