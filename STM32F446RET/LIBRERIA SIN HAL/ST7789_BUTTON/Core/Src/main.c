...
*/
  ...
  {
	  k_inkey = KEYB_Inkeys();
	  if (k_inkey & (1<<KEYB_LEFT)) {write_sym(lcd, "a");}
	  if (k_inkey & (1<<KEYB_RIGHT)){write_sym(lcd, "b");}
	  if (k_inkey & (1<<KEYB_UP))   {write_sym(lcd, "c");}
	  if (k_inkey & (1<<KEYB_DOWN)) {write_sym(lcd, "d");}
	  //LL_mDelay(200);
  }
