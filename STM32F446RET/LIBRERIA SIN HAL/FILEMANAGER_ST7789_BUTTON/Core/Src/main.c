...
/* USER CODE BEGIN PFP */
void write_sym(LCD_Handler *lcd, char *str){
	LCD_WriteString(lcd, lcd->AtPos.x, lcd->AtPos.y, str, &Font_15x25, COLOR_WHITE, COLOR_BLACK, LCD_SYMBOL_PRINT_FAST);
	return;
}
/* USER CODE END PFP */
...
int main(void)
{
  ...
  TIM10->DIER |= TIM_DIER_UIE;//Habilita la interrupción por desbordamiento (update interrupt)
  TIM10->CR1  |= TIM_CR1_CEN;//Inicia (habilita) el temporizador, esto hace que el timer empiece a contar.
...
/*  Для дисплея на контроллере ST7789   */
  LCD = LCD_DisplayAdd( LCD,
						...
						-35,		//смещение по ширине дисплейной матрицы  -35//-53
						  0,		//смещение по высоте дисплейной матрицы    0// 40
...
*/
  ...
  {
	  k_inkey = KEYB_Inkeys();
	  if (k_inkey & (1<<KEYB_LEFT)) {write_sym(lcd, "E");}
	  if (k_inkey & (1<<KEYB_RIGHT)){write_sym(lcd, "F");}
	  if (k_inkey & (1<<KEYB_UP))   {write_sym(lcd, "G");}
	  if (k_inkey & (1<<KEYB_DOWN)) {write_sym(lcd, "6");}
	  //LL_mDelay(200);
  }
...
static void MX_GPIO_Init(void)
{
  ...
  /**/
  LL_GPIO_ResetOutputPin(GPIOA, LCD_DC_Pin|LCD_RES_Pin|LCD_CS_Pin);
