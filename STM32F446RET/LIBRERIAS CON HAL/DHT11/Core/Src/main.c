...
/* USER CODE BEGIN PV */
uint8_t temp, hum; // creo dos variables para leer los datos de T° y H
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	while (1) {
		...
		if(hum>50){
			HAL_GPIO_TogglePin(ledBoard_GPIO_Port, ledBoard_Pin);
			HAL_Delay(250);
		}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
	if (htim->Instance == TIM6) {//Leemos el sensor cada 1200ms
		Leer_DHT11(&temp, &hum);
	}
}
/* USER CODE END 4 */
