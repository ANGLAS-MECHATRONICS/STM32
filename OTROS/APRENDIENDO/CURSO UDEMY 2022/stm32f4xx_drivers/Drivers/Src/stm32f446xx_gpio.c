...
void GPIO_Init(GPIO_handle_t *pGPIOHandle){
	...
	if(pGPIOHandle->GPIO_config.GPIO_Mode > GPIO_MODE_AN){
		...
		regidx = pGPIOHandle->GPIO_config.GPIO_Pin / 4;  //0=EXTICR[0]  1=EXTICR[1] 2=EXTICR[2] 3=EXTICR[3]
		blkidx = pGPIOHandle->GPIO_config.GPIO_Pin % 4;

		SYSCFG->EXTICR[regidx] &=~ (0x0F << (blkidx*4));//primero, reseteamos los bits
	...
	if(pGPIOHandle->GPIO_config.GPIO_Mode == GPIO_MODE_AF){
		...
		regidx = pGPIOHandle->GPIO_config.GPIO_Altfun / 8;  //0=AFR[0]    1=AFR[1]
...
void GPIO_NVIC_Priority(uint8_t IRQ, uint8_t priority){
	...
	*NVIC_IPR &=~ (0xFF << (blkidx * 8));//primero limpiamos los 4 bits
	*NVIC_IPR &=~ (priority << (blkidx * 8 + 4));
}

void GPIO_IRQHandler(uint8_t pin){
	if(EXTI->PR & (1 << pin))
		EXTI->PR |= (1 << pin); //primero limpiamos el bit, se limpia con 1
}
